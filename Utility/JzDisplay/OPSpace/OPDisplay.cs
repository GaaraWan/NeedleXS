...
namespace JzDisplay.OPSpace
{
    class OPDisplay
    {
        ...
        public event EventHandler<DisplayEventArgs> OnFireMessage;
        ...
        Bitmap bmpOrgPixel = new Bitmap(1, 1);   //原來的的圖形
        ...
        Pen m_DsRectCenter = new Pen(Color.Orange, 1);
        ...
        private void FillInformation()
        {
            ...
            str += " " + MyControlPara.MyControlMode.ToString() + " ";
            ...
            {
                //bmpOrgPixel.Dispose();
                //bmpOrgPixel = (Bitmap)bmpOrg.Clone();
                //Color mcolor = bmpOrgPixel.GetPixel((int)ptfmapping.X, (int)ptfmapping.Y);
                Color mcolor = bmpOrg.GetPixel((int)ptfmapping.X, (int)ptfmapping.Y);
                //Color mcolor = Color.FromArgb(0, 0, 0);// bmpOrg.GetPixel((int)ptfmapping.X, (int)ptfmapping.Y);
        ...
        private void PicDisplay_Paint(object sender, PaintEventArgs e)
        {
            ...
            //中心框
            PointF dsCenter = new PointF(picDisplay.Width / 2.0f, picDisplay.Height / 2.0f);
            Rectangle dsRectCenter = SimpleRect(dsCenter, 10);
            m_DsRectCenter.Color = Color.Orange;
            m_DsRectCenter.DashStyle = DashStyle.DashDotDot;
            //grfx.DrawRectangle(m_DsRectCenter, dsRectCenter);
            grfx.DrawLine(new Pen(Color.Yellow, 1), new PointF(0, picDisplay.Height / 2.0f), new PointF(picDisplay.Width, picDisplay.Height / 2.0f));
            grfx.DrawLine(new Pen(Color.Yellow, 1), new PointF(picDisplay.Width / 2.0f, 0), new PointF(picDisplay.Width / 2.0f, picDisplay.Height));

            //PointF dsCenter1 = new PointF(picDisplay.Width / 2.0f, picDisplay.Height / 2.0f);
            Rectangle dsRectCenter1 = SimpleRect(ptMouse_Move, 20);
            m_DsRectCenter.Color = Color.Lime;
        ...
        private void PicDisplay_MouseDown(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left)
            {
                PointF ptfmapping = ptfMapping(e.Location);
                PointF ptOrgCenter = new PointF(bmpOrg.Width / 2, bmpOrg.Height / 2);
                PointF ptOffset = new PointF(-(float)((ptfmapping.X - ptOrgCenter.X)),
                                                                  -(float)((ptfmapping.Y - ptOrgCenter.Y)));
                JRotatedRectangleF jRotatedRectangleF = new JRotatedRectangleF();
                switch (MyControlPara.MyControlMode)
                {
                    case ControlMode.MoveOffset:
                        //输出偏移量
                        //OnClickDebug(ptOffset, e.Button);
                        FireDisplayMessage(new DisplayEventArgs("MoveOffset", ptOffset));
                        break;
                    case ControlMode.MoveMaxBlob:
                        ...
                        break;
                    case ControlMode.MoveMaxBlobPos:
                        //PointF ptfmappingtemp = getMoveBlobOffset(ptfmapping, out jRotatedRectangleF);
                        //ptOffset = new PointF(-(float)((ptfmappingtemp.X - ptOrgCenter.X)),
                        //                                          -(float)((ptfmappingtemp.Y - ptOrgCenter.Y)));
                        //OnClickDebug(ptOffset, e.Button);
                        getMoveBlobOffset(ptfmapping, out jRotatedRectangleF);
                        FireDisplayMessage(new DisplayEventArgs("MoveMaxBlobPos", jRotatedRectangleF));
                        break;
                }
            }
        ...
        //public delegate void DebugClickPointHandler(PointF ptLocation, MouseButtons eMyMouseButtons);
        //public event DebugClickPointHandler DebugClickPointAction;
        //public void OnClickDebug(PointF ptLocation, MouseButtons eMouseButtons)
        //{
        //    if (DebugClickPointAction != null)
        //    {
        //        DebugClickPointAction(ptLocation, eMouseButtons);
        //    }
        //}

        protected void FireDisplayMessage(DisplayEventArgs e)
        {
            try
            {
                OnFireMessage?.Invoke(this, e);
            }
            catch (Exception ex)
            {
        ...
        private PointF getMoveBlobOffset(PointF ptLocation,out JRotatedRectangleF jRotatedRectangleF)
        {
            ...
            BoundRect(ref rectLocation, bmpOrg.Size);
            Bitmap bmptemp = bmpOrg.Clone(rectLocation, PixelFormat.Format24bppRgb);
